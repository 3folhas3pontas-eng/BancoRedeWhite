...
export default function LojasAbertasView({ onBack }: LojasAbertasViewProps) {
      ...
      <main className="flex-1 px-6 py-4 overflow-y-auto hide-scrollbar pb-32">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            <div className="w-10 h-10 border-4 border-[#72E8F6] border-t-transparent rounded-full animate-spin mb-4" />
              ...
              return (
                <div
                  ...
                  className={`relative p-5 rounded-[24px] border-2 transition-all duration-500 bg-white flex flex-col gap-3 shadow-sm ${
                    isOpen ? 'border-[#2ECC71] shadow-[#2ECC7115]' : 'border-[#EDEDED] opacity-60'
                  }`}
                >
                  <div className="flex justify-between items-start">
                    <div className="flex flex-col">
                      ...
                      <p className="text-gray-500 text-xs font-medium uppercase tracking-widest mt-1">
                        Dono: {shop.owner_name}
                      </p>
                    </div>
                    <div
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-black tracking-tighter ${
                        isOpen ? 'bg-[#2ECC7120] text-[#2ECC71]' : 'bg-[#E74C3C20] text-[#E74C3C]'
                      }`}
                    >
                      <span
                        className={`w-2 h-2 rounded-full ${isOpen ? 'bg-[#2ECC71] animate-pulse' : 'bg-[#E74C3C]'}`}
                      />
                      <span className={isOpen ? 'animate-pulse' : ''}>{shop.status}</span>
                    </div>
                  </div>

                  <div className="w-full h-[1px] bg-[#F5F6F7] mt-2" />
                  ...
                  <div className="flex items-center justify-between text-[11px] text-gray-400 font-bold mt-1">
                    ...
                    {isOpen && <span className="text-[#72E8F6]">Vá até a loja agora!</span>}
                  </div>
                </div>
              );
            })}
        ...
        <div className="mt-12 p-6 rounded-2xl bg-[#F5F6F7] border border-[#EDEDED] text-center">
          <p className="text-gray-500 text-xs font-medium leading-relaxed">
            As informações acima são atualizadas automaticamente via{' '}
            <span className="text-[#72E8F6]">RedeWhite Cloud</span>.<br />
